	...
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		orgs := []*github.OrganizationConfig{
			github.NewOrganizationConfig("nicklasfrahm", []github.RepositoryConfig{
				...
				github.NewRepositoryConfig("file-secret-action", "A GitHub Action to upload a file as a GitHub Actions Secret.", &github.RepositoryOptions{
					Topics: []string{"github", "secret", "file", "action"},
				}),
				github.NewRepositoryConfig("indesy-robot", "The robot for an indoor delivery system.", &github.RepositoryOptions{
					Topics: []string{"robot", "system", "indoor", "delivery", "indesy"},
				}),
				github.NewRepositoryConfig("indesy-server", "The server for an indoor delivery system.", &github.RepositoryOptions{
					Topics: []string{"system", "server", "indoor", "delivery", "indesy"},
				}),
				github.NewRepositoryConfig("indesy-webclient", "The web application for an indoor delivery system.", &github.RepositoryOptions{
					Topics: []string{"system", "webclient", "indoor", "delivery", "indesy"},
				}),
				github.NewRepositoryConfig("infrastructure", "A repository to automate the configuration of my local infrastructure.", &github.RepositoryOptions{
				...
				github.NewRepositoryConfig("mykilio", "Mykilio is the proposal for a new Living Standard with the goal to reimagine infrastructure management and monitoring. It aims to be scalable, lightweight, extensible and secure by applying IoT principles to the datacenter and homelabs.", &github.RepositoryOptions{
				...
				github.NewRepositoryConfig("node-iot-server", "A small server to make a serial device addressable over UDP.", &github.RepositoryOptions{
					Topics: []string{"iot", "serial", "server", "udp"},
				}),
				github.NewRepositoryConfig("odance", "O!Dance is a dancing school project in Sonderborg, Denmark.", &github.RepositoryOptions{
					Topics:      []string{"website", "school", "dancing"},
					HomepageUrl: "https://odance.dk",
					PagesSource: "gh-pages",
				}),
				github.NewRepositoryConfig("pulumi-ssh", "Manage a remote server declaratively via SSH.", &github.RepositoryOptions{
					Template: "pulumi/pulumi-provider-boilerplate",
				}),
				...
				github.NewRepositoryConfig("scp-action", "A Github Action to upload and download files via SCP.", &github.RepositoryOptions{
					Topics: []string{"github", "ssh", "scp", "action"},
				}),
				github.NewRepositoryConfig("ses", "Software for Embedded Systems Course Source Code", nil),
				github.NewRepositoryConfig("showcases", "A repository containing showcases for my programming and architecture skills.", &github.RepositoryOptions{
			...
			github.NewOrganizationConfig("mykilio", []github.RepositoryConfig{
				github.NewRepositoryConfig("docs", "A collection of documentation for all components of the Mykilio ecosystem.", &github.RepositoryOptions{
					HomepageUrl: "https://docs.mykil.io",
					Topics:      []string{"documentation", "microservices", "mykilio"},
					PagesSource: "docs",
				}),
				github.NewRepositoryConfig("mykilio.go", "The primary SDK for integrating with the ecosystem.", &github.RepositoryOptions{
